...
- Creeu els enllaços simbòlics a `/etc/nginx/sites-enabled/` per activar les configuracions.
- Verifiqueu la sintaxi amb la comanda `nginx -t` abans de reiniciar el servei.
...
- Configureu el Server Block per escoltar al port 443 i indiqueu les rutes del certificat amb `ssl_certificate` i la clau privada amb `ssl_certificate_key`.

### Redirecció forçada
Configureu un bloc de servidor escoltant al port 80 que retorni un codi **301 (Permanent Redirect)** cap a la versió HTTPS del domini `projectenexus.test` o `academia.test`.

## 5. Optimització amb HTTP/2
- Habiliteu el protocol HTTP/2 afegint el paràmetre `http2` a la directiva `listen` del bloc SSL.
- Exemple: `listen 443 ssl http2;`
